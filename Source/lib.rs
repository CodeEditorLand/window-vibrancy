...
//! ## Platform-specific
//!
//! - **Linux**: Unsupported, Blur and any vibrancy effects are controlled by
//!   the compositor installed on the end-user system.
//!
//! # Example
//!
//! ```no_run
...
/// Applies blur effect to window. Works only on Windows 7, Windows 10 v1809 or
/// newer.
///
/// ## WARNING:
///
...
/// Clears blur effect applied to window. Works only on Windows 7, Windows 10
/// v1809 or newer.
///
/// ## Platform-specific
///
...
/// ## WARNING:
///
/// This method has poor performance on Windows 10 v1903+ and Windows 11 build
/// 22000, the window will lag when resizing or dragging.
/// It is an issue in the undocumented api used for this method
/// and microsoft needs to fix it (they probably won't).
///
/// ## Platform-specific
...
/// Clears acrylic effect applied to window. Works only on Windows 10 v1809 or
/// newer.
///
/// ## Platform-specific
///
...
impl From<raw_window_handle::HandleError> for Error {
	fn from(err:raw_window_handle::HandleError) -> Self { Error::NoWindowHandle(err) }
}
